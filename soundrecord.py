...
parser.add_argument(
    '-s', '--split-after', type=int, default=2**63, help='split after this number of seconds')
...
try:
   ...
   ########
    ...
    with sd.InputStream(samplerate=args.samplerate, device=args.device,
        ...
        while True:
            now = dt.now()
            context = {'counter': counter, 'year': now.year, 'month': now.month, 'day': now.day, 'hour': now.hour, 'minute': now.minute, 'second': now.second}
